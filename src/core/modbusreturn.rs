...
fn test_modbus_return_coils_on_bad() {
  let result: ModbusReturnCoils = ModbusReturnCoils::Bad(ReturnBad::new_with_codes(0x01, 0x01));
...
fn test_modbus_return_coils_on_good() {
  ...
  let result: ModbusReturnCoils = ModbusReturnCoils::Good(ReturnGood::new(test_data, 250));
  assert_eq!(result.is_none(), false);
  assert_eq!(result.is_some(), true);
...
fn test_modbus_return_registers_on_bad() {
  let result: ModbusReturnRegisters = ModbusReturnRegisters::Bad(ReturnBad::new_with_codes(0x01, 0x01));
  assert_eq!(result.is_none(), false);
  assert_eq!(result.is_some(), true);
  assert_eq!(result.is_bad(), true);
  assert_eq!(result.is_good(), false);
...
fn test_modbus_return_registers_on_good() {
  ...
  let result: ModbusReturnRegisters = ModbusReturnRegisters::Good(ReturnGood::new(test_data, 250));
  assert_eq!(result.is_none(), false);
  assert_eq!(result.is_some(), true);
  assert_eq!(result.is_bad(), false);
  assert_eq!(result.is_good(), true);
...
fn test_return_bad_by_new_with_codes() {
  let result: ReturnBad = ReturnBad::new_with_codes(0x01, 0x01);
...
fn test_return_good() {
  ...
  let mut result: ReturnGood<u16> = ReturnGood::new(test_data, 4);
