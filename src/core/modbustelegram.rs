  ...
  pub fn new(
    ...
    if transaction_identifier > 0x0000 && function_code > 0x00 {
      ...
      return Result::Err(ModbusTelegramError {
        message: "Transaction ID and function code were both zero".to_string(),
      });
    }
  }
  ...
  pub fn new_from_bytes(bytes: &Vec<u8>) -> Result<ModbusTelegram, DataTransformError> {
    ...
    if bytes.len() > 9 {
      ...
      return Result::Err(DataTransformError {
        message: "Byte array length must be greater than 9".to_string(),
      });
    }
  }
...
fn test_extract_payload_by_function_code() {
  ...
  let result_data_1: Result<Vec<u8>, DataTransformError> = extract_payload_by_function_code(0x03, &test_data_1);
...
fn extract_payload_by_function_code(function_code: u8, bytes: &Vec<u8>) -> Result<Vec<u8>, DataTransformError> {
  ...
  match function_code {
    0x01 => {
      reply = extract_payload_with_byte_count(&bytes)?;
    }
    0x02 => {
      reply = extract_payload_with_byte_count(&bytes)?;
    }
    ...
    0x05 => {
      reply = extract_payload_without_byte_count(&bytes)?;
    }
    0x06 => {
      reply = extract_payload_without_byte_count(&bytes)?;
    }
    ...
    _ => {
      return Result::Err(DataTransformError {
        message: "Could not match function code ".to_string(),
      });
    }
  }
  Ok(reply)
...
fn test_verify_function_code() {
  ...
  let test_data_request: Result<ModbusTelegram, ModbusTelegramError> =
    ModbusTelegram::new(0x00A0, 0x01, 0x01, &l_payload, 0x00);
  assert!(test_data_request.is_ok());

  let test_data_response: Result<ModbusTelegram, ModbusTelegramError> =
    ModbusTelegram::new(0x00A0, 0x01, 0x01, &l_payload, 0x00);
  assert!(test_data_response.is_ok());

  let is_equal: bool = verify_function_code(&test_data_request.unwrap(), &test_data_response.unwrap());
  assert!(is_equal);
}
