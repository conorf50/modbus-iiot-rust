...
fn test_append_byte_to_bytearray() {
  ...
  append_byte_to_bytearray(&mut test_array, byte);
...
fn test_append_bytearray_to_bytearray() {
  ...
  append_bytearray_to_bytearray(&mut test_array_target, &test_array_source);
...
pub fn append_bytearray_to_bytearray(target_array: &mut Vec<u8>, source_array: &Vec<u8>) {
  for byte in source_array {
    append_byte_to_bytearray(target_array, *byte);
  }
}
...
fn test_append_word_to_bytearray() {
  ...
  append_word_to_bytearray(&mut test_array, word);
  assert_eq!(test_array.len(), 4);
...
fn test_extract_byte_from_bytearray() {
  ...
  let result_1: Result<u8, DataTransformError> = extract_byte_from_bytearray(&test_array, 0);
  assert!(result_1.is_ok());
  assert_eq!(result_1.unwrap(), 0x01);

  let result_2: Result<u8, DataTransformError> = extract_byte_from_bytearray(&test_array, 2);
  assert!(result_2.is_ok());
  assert_eq!(result_2.unwrap(), 0xFF);

  let result_3: Result<u8, DataTransformError> = extract_byte_from_bytearray(&test_array, 3);
  assert!(result_3.is_ok());
  assert_eq!(result_3.unwrap(), 0x10);

  let result_4: Result<u8, DataTransformError> = extract_byte_from_bytearray(&test_array, 4);
...
fn test_extract_bytes_from_bytearray() {
  ...
  let result_1: Result<Vec<u8>, DataTransformError> = extract_bytes_from_bytearray(&test_array, 0, 1);
  assert!(result_1.is_ok());
  assert_eq!(result_1.unwrap()[0], 0x01);

  let result_2: Result<Vec<u8>, DataTransformError> = extract_bytes_from_bytearray(&test_array, 1, 2);
  assert!(result_2.is_ok());
  ...
  let result_3: Result<Vec<u8>, DataTransformError> = extract_bytes_from_bytearray(&test_array, 1, 4);
  assert!(result_3.is_err());

  let result_4: Result<Vec<u8>, DataTransformError> = extract_bytes_from_bytearray(&test_array, 3, 1);
  assert!(result_4.is_ok());
  assert_eq!(result_4.unwrap()[0], 0x10);

  let result_5: Result<Vec<u8>, DataTransformError> = extract_bytes_from_bytearray(&test_array, 3, 2);
  assert!(result_5.is_err());

  let result_6: Result<Vec<u8>, DataTransformError> = extract_bytes_from_bytearray(&test_array, 4, 1);
  assert!(result_6.is_err());
}

pub fn extract_bytes_from_bytearray(
  source_array: &Vec<u8>,
  start_index: u8,
  byte_count: u8,
  ...
  if verify_length <= source_length {
    ...
    if copy_array.len() > 0 {
      ...
      return Result::Err(DataTransformError {
        message: "Byte array length must be greater than 0".to_string(),
      });
    }
  } else {
    return Result::Err(DataTransformError {
      message: "Error parsing byte array".to_string(),
    });
  }
}
...
fn test_extract_word_from_bytearray() {
  ...
  let result_2: Result<u16, DataTransformError> = extract_word_from_bytearray(&test_array, 3);
...
fn test_transform_bytes_to_word() {
  ...
  let result: u16 = transform_bytes_to_word(&test_vec, 0);
...
fn test_transform_bytes_to_words() {
  ...
  let result_1: Vec<u16> = transform_bytes_to_words(&test_array, 0, 2);
  assert_eq!(result_1.len(), 2);
  assert_eq!(result_1[0], 0x010A);
  assert_eq!(result_1[1], 0xF00F);

  let result_2: Vec<u16> = transform_bytes_to_words(&test_array, 2, 2);
  assert_eq!(result_2.len(), 2);
  assert_eq!(result_2[0], 0xF00F);
  assert_eq!(result_2[1], 0x0A10);

  let result_3: Vec<u16> = transform_bytes_to_words(&test_array, 6, 1);
  assert_eq!(result_3.len(), 1);
  assert_eq!(result_3[0], 0xFFFF);

  let result_4: Vec<u16> = transform_bytes_to_words(&test_array, 6, 2);
...
pub fn transform_bytes_to_words(databytes: &Vec<u8>, start_index: u8, word_count: u8) -> Vec<u16> {
  ...
  if verify_length <= source_length {
    ...
    for _ in 0..word_count {
      word = transform_bytes_to_word(databytes, working_index);
      working_index += 2;
